...
function CardPermiso(props) {
  const { obtenerPermisosActualizados } = props;
  ...
  const handleAprobarClick = async (e) => {
    try {
      if (
        ...
        await update("Pendiente", "Pendiente de aprobación de dirección");
      } else if (props.observaciones === "Pendiente de aprobación de tutor") {
  ...
  const handleRechazarClick = async (e) => {
    try {
      await update("Rechazado", "Rechazado por tutor");
