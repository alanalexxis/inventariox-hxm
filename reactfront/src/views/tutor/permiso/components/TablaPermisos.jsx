...
import CardPermiso from "components/card/CardPermisoTutor";

import CardFiltroTutor from "components/card/CardFiltroTutor";
...
const TablaPermisos = ({ transparent, onFilterAprobados }) => {
  ...
  const deletePermiso = async (idpermisos) => {
    try {
      ...
      const nombreTutor = data.tutor.nombre;
      await axios.delete(`${URI}${idpermisos}`);
      const res = await axios.get(URI);
      const permisos = res.data;

      const permisosFiltrados = permisos.filter(
        (permiso) => permiso.tutor === data.tutor.nombre
      );
      setPermisosFiltrados(permisosFiltrados);
      setOpen(false);
  ...
  const obtenerPermisosActualizados = async () => {
    try {
      ...
      const res = await axios.get(`${URI}?tutor=${nombreTutor}`);
      const permisos = res.data;
  ...
  return (
    <Card extra={"w-full pb-10 p-4 h-full"} style={{ marginTop: "50px" }}>
      ...
      <header className="relative flex items-center justify-between">
        ...
        />
        <CardFiltroTutor onFilterAprobados={handleFilterAprobados} />
      </header>
                    ...
                    <td className="text-sm font-bold text-navy-700 dark:text-white">
                      <div className="flex items-center gap-2">
                        {((permiso.status === "Pendiente" &&
                          ...
                          permiso.observaciones ===
                            "Pendiente de aprobación de tutor") && (
