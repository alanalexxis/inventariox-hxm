...
const TablaTutores = (props) => {
  ...
  const [idtutoresToDelete, setIdTutoresToDelete] = useState(null);

  const [tutores, setTutor] = useState([]);
  useEffect(() => {
    getTutores();
  }, []);
  ...
  const deleteTutor = async (idtutores) => {
    await axios.delete(`${URI}${idtutores}`);
    setOpen(false);
    setIdTutoresToDelete(null);
    getTutores();
  };

  const handleDeleteClick = (idtutores) => {
    setIdTutoresToDelete(idtutores);
    setOpen(true);
  };
  ...
  return (
    <Card extra={"w-full pb-10 p-4 h-full"} style={{ marginTop: "50px" }}>
      ...
      >
        Registrar tutor <i className="fas fa-user-plus mr-0"></i>
      </Link>
            ...
            {tutores.map((tutor) => (
              <tr>
                ...
                <td className="flex items-center">
                  <Link to={`/admin/tutor/edit/${tutor.idtutores}`}>
                    <FaEdit className="text-gray-400 hover:text-gray-800" />
                  </Link>

                  <button
                    onClick={() => handleDeleteClick(tutor.idtutores)}
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                      ...
                                      <div className="mt-2">
                                        <p className="text-sm text-gray-500 dark:text-gray-300">
                                          ...
                                          este tutor? Todos sus datos serán
