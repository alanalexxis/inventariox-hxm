...
const TablaAlumnosSinGrupo = (props) => {
  const [open, setOpen] = useState(false);
  const cancelButtonRef = useRef(null);

  const [alumnos, setAlumno] = useState([]);
  const [idalumnosToDelete, setIdAlumnosToDelete] = useState(null);
  useEffect(() => {
    getAlumnos();
  }, []);
  ...
  const deleteAlumno = async (idalumnos) => {
    await axios.delete(`${URI}${idalumnos}`);
    setOpen(false);
    setIdAlumnosToDelete(null);
    getAlumnos();
  };
  const handleDeleteClick = (idalumnos) => {
    setIdAlumnosToDelete(idalumnos);
    setOpen(true);
  };
  ...
  return (
    <Card extra={"w-full pb-10 p-4 h-full"} style={{ marginTop: "50px" }}>
      ...
      >
        Registrar alumno <i className="fas fa-user-plus mr-0"></i>
      </Link>
            ...
            {alumnos.map((alumno) => (
              <tr>
                ...
                <td className="flex items-center">
                  <Link to={`/admin/alumno/edit/${alumno.idalumnos}`}>
                    <FaEdit className="text-gray-400 hover:text-gray-800" />
                  </Link>

                  <button
                    onClick={() => handleDeleteClick(alumno.idalumnos)}
                              ...
                              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all dark:bg-gray-800 sm:my-8 sm:w-full sm:max-w-lg">
                                ...
                                <div className="bg-gray-50 px-4 py-3 dark:bg-gray-800 sm:flex sm:flex-row-reverse sm:px-6">
                                  <button
                                    ...
                                    onClick={() =>
                                      deleteAlumno(idalumnosToDelete)
                                    }
                                  >
                                    Eliminar
                                  </button>
...
export default TablaAlumnosSinGrupo;
