...
const TablaCarreras = (props) => {
  const [open, setOpen] = useState(false);
  const cancelButtonRef = useRef(null);
  const [idcarrerasToDelete, setIdCarrerasToDelete] = useState(null);
  ...
  const deleteCarrera = async (idcarreras) => {
    await axios.delete(`${URI}${idcarreras}`);
    setOpen(false);
    setIdCarrerasToDelete(null);
    getCarreras();
  };

  const handleDeleteClick = (idcarreras) => {
    setIdCarrerasToDelete(idcarreras);
    setOpen(true);
  };
  ...
  return (
    <Card extra={"w-full pb-10 p-4 h-full"} style={{ marginTop: "50px" }}>
      ...
      >
        Registrar carrera <i className="fas fa-book-open mr-0"></i>
      </Link>
            ...
            {carreras.map((carrera) => (
              <tr>
                ...
                <td className="flex items-center">
                  <Link to={`/admin/carrera/edit/${carrera.idcarreras}`}>
                    <FaEdit className="text-gray-400 hover:text-gray-800" />
                  </Link>

                  <button
                    onClick={() => handleDeleteClick(carrera.idcarreras)}
                              ...
                              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all dark:bg-gray-800 sm:my-8 sm:w-full sm:max-w-lg">
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                      ...
                                      <div className="mt-2">
                                        <p className="text-sm text-gray-500 dark:text-gray-300">
                                          ...
                                          esta carrera? Todos sus datos serán
                                ...
                                <div className="bg-gray-50 px-4 py-3 dark:bg-gray-800 sm:flex sm:flex-row-reverse sm:px-6">
                                  <button
                                    ...
                                    onClick={() =>
                                      deleteCarrera(idcarrerasToDelete)
                                    }
                                  >
                                    Eliminar
                                  </button>
