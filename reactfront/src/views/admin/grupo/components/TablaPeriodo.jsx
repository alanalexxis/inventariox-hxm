...
const TablaPeriodos = (props) => {
  ...
  const [idperiodosToDelete, setIdPeriodosToDelete] = useState(null);
  ...
  const deletePeriodo = async (idperiodos) => {
    await axios.delete(`${URI}${idperiodos}`);
    setOpen(false);
    setIdPeriodosToDelete(null);
    getPeriodos();
  };

  const handleDeleteClick = (idperiodos) => {
    setIdPeriodosToDelete(idperiodos);
    setOpen(true);
  };
  ...
  return (
    <Card extra={"w-full pb-10 p-4 h-full"} style={{ marginTop: "50px" }}>
      ...
      >
        Registrar periodo <i className="fas fa-calendar-plus mr-0"></i>
      </Link>
            ...
            {periodos.map((periodo) => (
              <tr>
                ...
                <td className="flex items-center">
                  <Link to={`/admin/periodo/edit/${periodo.idperiodos}`}>
                    <FaEdit className="text-gray-400 hover:text-gray-800" />
                  </Link>

                  <button
                    onClick={() => handleDeleteClick(periodo.idperiodos)}
                              ...
                              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all dark:bg-gray-800 sm:my-8 sm:w-full sm:max-w-lg">
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                      ...
                                      <div className="mt-2">
                                        <p className="text-sm text-gray-500 dark:text-gray-300">
                                          ...
                                          este periodo? Todos sus datos serán
                                ...
                                <div className="bg-gray-50 px-4 py-3 dark:bg-gray-800 sm:flex sm:flex-row-reverse sm:px-6">
                                  <button
                                    ...
                                    onClick={() =>
                                      deletePeriodo(idperiodosToDelete)
                                    }
                                  >
                                    Eliminar
                                  </button>
